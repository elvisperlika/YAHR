"""The Ranker's A2A executor: drives the task lifecycle, relaying core.rank."""
...
from yahr.agents.ranker import core
...
class RankerExecutor(AgentExecutor):
    """Runs core.rank as an A2A task, streaming progress until it completes."""
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        Args:
            context: The incoming A2A request (carries the orchestrator's bundle:
                question + found jobs + optional resume).
        ...
        console.log(f"[bold cyan]rank[/] {message.splitlines()[0]!r}")
